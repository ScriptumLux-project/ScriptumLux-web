  ...
  useEffect(() => {
    const fetchMovieData = async () => {
      try {
        ...
        // Try to fetch specific movie directly
        let movieData;
        
        try {
          // First try getting the movie from the specific endpoint
          movieData = await getMovieDetails(movieId);
        } catch (fetchError) {
          console.error('Error fetching specific movie:', fetchError);
          
          // If that fails, get all movies and find the specific one
          console.log('Attempting fallback: fetching from all movies');
          const allMovies = await getMovies();
          movieData = allMovies.find(m => 
            m.movieId === parseInt(movieId) || 
        ...
        const processedMovie = {
          ...movieData,
          id: movieData.movieId || movieData.id, // Handle different ID field names
          posterUrl: movieData.posterUrl || '/api/placeholder/300/450',
          rating: movieData.rating || 0,
          // Handle genre data structure differences
          genres: movieData.genre 
            ? [{ id: movieData.genreId, name: movieData.genre }] 
            : movieData.genres || [],
          releaseDate: movieData.releaseYear ? `${movieData.releaseYear}` : 'Unknown',
        ...
        // Fetch comments - wrapped in try/catch to not fail the whole page
        try {
              ...
              <div className="detail-value">
                {movie.genres && movie.genres.length > 0 ? (
                  movie.genres.map(genre => (
                    <span key={typeof genre === 'object' ? genre.id : genre} className="genre-badge">
                ...
                <div className="comment-content-container">
                  <div className="comment-header">
                    <span className="comment-user">
                      ...
                      <span className="comment-date"> — {comment.date || (comment.createdAt ? new Date(comment.createdAt).toISOString().split('T')[0] : 'Unknown')}</span>
                    </span>
                  </div>
