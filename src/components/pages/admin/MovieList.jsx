...
import {deleteMovie, getGenres} from '../../../api';

const MovieList = () => {
    const navigate = useNavigate();
    const {movies: apiMovies, loading: moviesLoading, error: moviesError} = useMovies();
    ...
    const formatGenres = genreId => {
        const g = genres.find(x => x.genreId === genreId);
        return g ? g.name : 'N/A';
    };
    ...
    if (moviesError) {
        return <h1>Error loading movies: {moviesError}</h1>;
    }

    if (genresError) {
        return <h1>Error loading genres: {genresError}</h1>;
    }
                    ...
                    <table className="users-table">
                        ...
                        {movies.map(movie => (
                            <tr key={movie.id}>
                                ...
                                <td>{formatGenres(movie.genreId)}</td>
